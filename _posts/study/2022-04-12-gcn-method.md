...
* Local neighborhood 구조에 대한 **오버피팅 문제를 완화**할 수 있다.
  *   $$K=3$$인 layer 한개를 사용했을 때와 $$K=1$$인 layer 세개를 사용했을 때를 생각해보자. 후자는 각 이웃이 연결되어 있지 않아도 정보가 들어올 수 있다. 반면 전자는 명시된 neighbor들의 정보만 사용한다. 상대적으로 더 다양한 정보를 받는 후자가 오버피팅 문제가 덜 발생할 것이다.
